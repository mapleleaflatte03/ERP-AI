...
def _collect_references(
    ...
    if ledger_stats.get("entry_count", 0) > 0:
        references.append({
            ...
            "period": {
                "start": ledger_stats.get("first_entry_date").isoformat() if ledger_stats.get("first_entry_date") else None,
                "end": ledger_stats.get("last_entry_date").isoformat() if ledger_stats.get("last_entry_date") else None
            }
        })
...
async def create_insight_record(
    ...
    """, insight_id, tenant_id, window_days, json.dumps(inputs or {}))
...
async def update_insight_status(
    ...
    if status == "running":
        ...
        await conn.execute("""
            ...
            WHERE id = $1
        """, insight_id, status, json.dumps(result) if result else None)
    elif status == "failed":
        await conn.execute("""
            UPDATE cfo_insights 
            SET status = $2, error_message = $3, completed_at = NOW()
            WHERE id = $1
        """, insight_id, status, error_message)
...
async def process_insight_async(conn, insight_id: uuid.UUID, tenant_id: uuid.UUID, window_days: int = 30) -> dict:
    ...
    try:
        ...
        """, uuid.uuid4(), insight_id, str(tenant_id), json.dumps({
