...
async def run_document_pipeline(job_id: str, file_path: str, file_info: dict[str, Any]) -> str:
    ...
    try:
        ...
        await update_job_state(
            conn, job_id, JobState.EXTRACTED,
            checkpoint_data={"text_length": len(text)},
            request_id=request_id,
        )
        await track_zone_entry(
            conn, job_id=job_id, zone=DataZone.EXTRACTED, tenant_id=str(tenant_uuid),
            document_id=job_id, extracted_text_preview=text[:4000],
            byte_count=len(text.encode("utf-8")), processing_time_ms=ocr_latency_ms,
            request_id=request_id,
        )
        await append_audit_event(
            conn, job_id=job_id, tenant_id=str(tenant_uuid), event_type="extracted",
            ...
            try:
                ...
                if embedding:
                    ...
                    qdrant_client.upsert_documents(
                        ...
                        metadatas=[{
                            "job_id": job_id, "tenant_id": str(tenant_uuid),
                            "filename": file_info.get("filename", "unknown"),
                            "doc_type": "invoice", "source": "worker",
                        }],
...
Trả về JSON theo format đã định."""
        ...
        await append_audit_event(
            conn, job_id=job_id, tenant_id=str(tenant_uuid), event_type="llm_proposed",
            event_data={"model": model_name, "llm_latency_ms": llm_latency_ms,
                        "confidence": proposal.get("confidence"), "doc_type": proposal.get("doc_type")},
            actor="llm", request_id=request_id,
        )

        # =========== STEP 3: Policy Evaluation ===========
        policy_result = await policy_evaluate(
            conn, proposal=proposal, job_id=job_id,
            tenant_id=str(tenant_uuid), request_id=request_id,
        )

        await append_audit_event(
            conn, job_id=job_id, tenant_id=str(tenant_uuid), event_type="policy_evaluated",
            event_data={
            ...
            actor="policy_engine", request_id=request_id,
        )

        # Create audit evidence
        await create_audit_evidence(
            conn, job_id=job_id, tenant_id=str(tenant_uuid), request_id=request_id,
            document_id=job_id, raw_file_uri=file_info.get("path"),
            extracted_text=text, prompt_version="v1", model_name=model_name,
            llm_stage="direct", llm_input=user_prompt, llm_output_json=proposal,
            llm_output_raw=str(response), llm_latency_ms=llm_latency_ms,
            validation_errors=proposal.get("risks", []),
            risk_flags=proposal.get("risks", []), decision="proposed",
        )

        await track_zone_entry(
            conn, job_id=job_id, zone=DataZone.PROPOSED, tenant_id=str(tenant_uuid),
            document_id=job_id, processing_time_ms=llm_latency_ms, request_id=request_id,
        )
        ...
        if needs_approval:
            ...
            await append_audit_event(
                conn, job_id, str(tenant_uuid), "needs_approval",
                {"reason": f"Policy result: {policy_result.overall_result.value}",
                 "rules_failed": policy_result.rules_failed},
                "policy_engine", request_id,
            )
            await update_audit_decision(conn, job_id, "waiting_approval", policy_result.overall_result.value, request_id)
        ...
        await track_zone_entry(
            ...
            document_id=job_id, proposal_id=persist_result.get("proposal_id"),
        ...
        await publish_event(
            ...
            payload={
                "job_id": job_id, "proposal_id": persist_result.get("proposal_id"),
                "ledger_entry_id": persist_result.get("ledger_id"),
                "invoice_no": proposal.get("invoice_no"), "vendor": proposal.get("vendor"),
                "total_amount": proposal.get("total_amount"), "currency": "VND",
            },
            tenant_id=str(tenant_uuid), request_id=request_id,
        )
        ...
        if conn:
            try:
                ...
                await append_audit_event(
                    conn, job_id, t_id, "failed", {"error": str(e)[:1000]}, "worker", request_id,
                )
