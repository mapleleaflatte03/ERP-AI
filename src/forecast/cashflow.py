...
async def compute_cashflow_forecast(
    ...
    for i in range(1, window_days + 1):
        ...
        daily_forecast.append({"date": forecast_date.isoformat(), "expected_net": float(round(avg_daily_net, 2))})
    ...
    forecast = {
        ...
        "summary": {"total_expected_net": total_expected_net, "historical_data_points": len(daily_flows)},
    }

    return forecast


async def _get_daily_cashflows(conn, tenant_id: uuid.UUID, start_date: date, end_date: date) -> list[Decimal]:
    """
...
async def persist_forecast(conn, tenant_id: uuid.UUID, forecast: dict[str, Any]) -> uuid.UUID:
    """
