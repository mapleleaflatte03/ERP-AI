...
async def run_benchmark(num_lines):
    ...
    await post_to_ledger(conn, proposal_id=str(uuid.uuid4()), approval_id=str(uuid.uuid4()), posted_by="benchmark_user")
    end_time = time.time()

    duration = end_time - start_time
    print(
        f"Lines: {num_lines:<4} | Duration: {duration:.4f}s | Execute calls: {conn.execute_count:<4} | Executemany calls: {conn.executemany_count}"
    )
