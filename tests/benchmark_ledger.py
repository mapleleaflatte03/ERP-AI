...
async def run_benchmark():
    ...
    for i in range(NUM_ENTRIES // 2):
        entries.append({"account_code": "111", "debit": 100.0, "credit": 0.0, "description": f"Debit {i}"})
        entries.append({"account_code": "222", "debit": 0.0, "credit": 100.0, "description": f"Credit {i}"})
    ...
    with patch("src.db.get_connection") as mock_get_conn:
        ...
        await post_ledger_entry(
            job_id="job-123", proposal_id="prop-123", entries=entries, description="Benchmark Transaction"
        )
