...
def count_db_records(token, endpoint):
    ...
    try:
        resp = requests.get(f"{KONG_URL}/api/{endpoint}", headers={"Authorization": f"Bearer {token}"}, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
...
def test_01_auth():
    ...
    resp = requests.get(f"{KONG_URL}/api/health", headers={"Authorization": f"Bearer {token}"})

    if resp.status_code == 200:
        log(f"Auth passed: Token {len(token)} chars, Kong verified", "PASS")
        results["tests"]["auth"] = {"status": "PASS", "token_length": len(token), "kong_status": resp.status_code}
        return token
    else:
...
def test_02_upload(token):
    ...
    log(f"Upload summary: {len(uploaded)} success, {len(failed)} failed", "PASS" if uploaded else "FAIL")
    return uploaded
...
def test_03_document_processing(token):
    ...
    resp = requests.get(f"{KONG_URL}/api/v1/jobs", headers={"Authorization": f"Bearer {token}"}, timeout=10)
    ...
    log(f"Doc Processing: {len(jobs)} jobs found, {len(processing_jobs)} in pipeline", "PASS" if jobs else "WARN")
    return jobs
...
def test_04_llm_langgraph(token):
    ...
    resp = requests.get(
        f"{KONG_URL}/api/v1/extracted-invoices", headers={"Authorization": f"Bearer {token}"}, timeout=10
    )
...
def test_05_journal_proposals(token):
    ...
    resp = requests.get(
        f"{KONG_URL}/api/v1/journal-proposals", headers={"Authorization": f"Bearer {token}"}, timeout=10
    )
...
def test_06_opa_guardrails(token):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            results["tests"]["opa_guardrails"] = {"status": "PASS", "policy_result": result}
...
def test_07_approvals(token):
    ...
    resp = requests.get(f"{KONG_URL}/api/v1/approvals", headers={"Authorization": f"Bearer {token}"}, timeout=10)
...
def test_08_ledger(token):
    ...
    resp = requests.get(f"{KONG_URL}/api/v1/ledger/entries", headers={"Authorization": f"Bearer {token}"}, timeout=10)
    ...
    results["tests"]["ledger"] = {"status": "PASS", "entries_count": len(entries)}
...
def test_09_reconciliation(token):
    ...
    resp = requests.get(f"{KONG_URL}/api/v1/reconciliation", headers={"Authorization": f"Bearer {token}"}, timeout=10)

    recon = {}
    if resp.status_code == 200:
        recon = resp.json()

    results["tests"]["reconciliation"] = {"status": "PASS", "data": recon if recon else "No active reconciliation"}

    log("Reconciliation: Endpoint accessible", "PASS")
...
def test_10_risk_anomaly(token):
    ...
    resp = requests.get(f"{KONG_URL}/api/v1/risk/alerts", headers={"Authorization": f"Bearer {token}"}, timeout=10)
    ...
    results["tests"]["risk_anomaly"] = {"status": "PASS", "alerts_count": len(alerts)}
...
def test_11_rag_qdrant(token):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            results["tests"]["rag_qdrant"] = {"status": "PASS", "collections": [c.get("name") for c in collections]}
    ...
    for i, prompt in enumerate(vietnamese_prompts[:3], 1):
        try:
            ...
            if resp.status_code == 200:
                ...
                log(
                    f"Copilot #{i}: {len(answer)} chars, VN={has_vietnamese}, Legal={has_legal_ref}",
...
# ==== MAIN ====
def main():
    log("═══ E2E FULL KAGGLE TEST START ═══")
    log(f"RUN_ID: {RUN_ID}")
    log(f"Base: {BASE}")

    # Count test files
    png_count = (
        len(list(Path(f"{BASE}/test_batch/images").glob("*.png"))) if os.path.exists(f"{BASE}/test_batch/images") else 0
    )
    pdf_count = (
        len(list(Path(f"{BASE}/test_batch/pdfs").glob("*.pdf"))) if os.path.exists(f"{BASE}/test_batch/pdfs") else 0
    )
    csv_count = (
        len(list(Path(f"{BASE}/test_batch/csv_excel").glob("*.csv")))
        if os.path.exists(f"{BASE}/test_batch/csv_excel")
        else 0
    )
    ...
    with open(md_path, "w") as f:
        f.write("# E2E Full Kaggle Test Report\n\n")
